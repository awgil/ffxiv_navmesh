...
public sealed class NavmeshBuilder
{
    ...
    public (DtMeshData?, Voxelizer?, RcBuilderResult) Build(CancellationToken token)
    {
        ...
        if (Settings.Filtering.HasFlag(NavmeshSettings.Filter.LowHangingObstacles))
        {
            ...
            Utils.Time("filter-low-hanging", () => RcFilters.FilterLowHangingWalkableObstacles(Telemetry, _walkableClimbVoxels, shf));
        }
        ...
        if (Settings.Filtering.HasFlag(NavmeshSettings.Filter.LedgeSpans))
        {
            ...
            Utils.Time("ledge-spans", () => RcFilters.FilterLedgeSpans(Telemetry, _walkableHeightVoxels, _walkableClimbVoxels, shf));
        }
        ...
        if (Settings.Filtering.HasFlag(NavmeshSettings.Filter.WalkableLowHeightSpans))
        {
            ...
            Utils.Time("low-height-spans", () => RcFilters.FilterWalkableLowHeightSpans(Telemetry, _walkableHeightVoxels, shf));
        }
        ...
        var chf = Utils.Time("chf", () => RcCompacts.BuildCompactHeightfield(Telemetry, _walkableHeightVoxels, _walkableClimbVoxels, shf));
        ...
        Utils.Time("erode", () => RcAreas.ErodeWalkableArea(Telemetry, _walkableRadiusVoxels, chf));
        ...
        {
            Utils.Time("monotone", () => RcRegions.BuildRegionsMonotone(Telemetry, chf, regionMinArea, regionMergeArea));
        }
        else
        {
            Utils.Time("partition-layers", () => RcRegions.BuildLayerRegions(Telemetry, chf, regionMinArea));
        }
        ...
        var cset = Utils.Time("contours", () => RcContours.BuildContours(Telemetry, chf, Settings.PolyMaxSimplificationError, polyMaxEdgeLenVoxels, RcBuildContoursFlags.RC_CONTOUR_TESS_WALL_EDGES));

        // 7. triangulate contours to build a mesh of convex polygons with adjacency information
        var pmesh = Utils.Time("polymesh", () => RcMeshs.BuildPolyMesh(Telemetry, cset, Settings.PolyMaxVerts));
        ...
        RcPolyMeshDetail? dmesh = Utils.Time("polymesh-detail", () => RcMeshDetails.BuildPolyMeshDetail(Telemetry, pmesh, chf, detailSampleDist, detailSampleMaxError));
